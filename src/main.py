...
def train():
    ...
    train_neg_path = 'dataset/train/output_neg'
...
if __name__ == '__main__':
    ...
    testfilename_list = testfilename.split('.')

    img = cv2.imread(testfilename)
    candidates = img_processing(img)
    X = extract_hog(candidates)
    preds = clf.predict(X)
    ...
    for i in range(len(preds)):
        if preds[i]:
            cv2.imwrite('out' + testfilename_list[0] + '_' + str(i) + '.' + testfilename_list[1], candidates[i])
...
#
#     test_frame = cv2.imread('150904-vacation-and-travel-road-traffic-signs-and-labeling-photocase-stock-photo-large.jpeg')
#     count = 0
#
